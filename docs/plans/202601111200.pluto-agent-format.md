...
- `${date}` — Current date
- `${time}` — Current time
- `${pluto.state_dir}` — Runtime state root (file-based mode uses `./.pluto/`)
...
- Use date-based Markdown files under `${pluto.state_dir}/memory/` (e.g. `YYYY-MM-DD.md`) for durable continuity notes.
